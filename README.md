...
**현재 버전: v0.6.15 (창 너비 검사를 실측 기준으로)**
...
| **v0.6.13** | 다음 상대 화면의 `본루타`·`도루`·닫는 괄호가 깨지던 문제. 라벨 풀이 16x16 한자 글리프를 덮어쓰고 있었다 | 완료 |
| **v0.6.14** | `map_labels`가 행을 한 번도 못 잡던 버그(주소 대신 상수를 읽고 있었다) 수정. 다음 상대 화면이 전부 라벨로 나온다 | 완료 |
| **v0.6.15** | 메시지 창 너비 검사를 실측 기준으로. 대사에 208px를 재던 잡음 경고 5건 제거, 창이 좁은 메시지는 진짜 넘침으로 검사 | 완료 |
...
### `map_labels`가 행을 한 번도 못 잡고 있었다 (v0.6.14)

- **증상.** 다음 상대 화면의 `次の対戦相手`가 `유 대전상대`로 나왔다. 글리프 교체(`KOREAN16`)는 글리프
  하나에 음절 하나만 줄 수 있는데 `次`는 응원 메뉴의 `[野次]` -> `[야유]`에 이미 `유`로 잡혀 있었다.
  `labels.csv`의 `map_93B7BC_01C8`에 `다음 대전 상대`라는 번역이 v0.5.9부터 들어 있었는데도 그렇다.
- **원인.** `map_labels`는 버퍼의 그 자리에 원본 워드가 아직 있는지로 자기 행을 알아본다. 그런데 그
  주소를 X에 넣는 코드가 `tax` 한 줄이었고, **그 시점 A에는 바로 윗줄의 `#$007F`가 들어 있었다.** 그래서
  늘 `$7F:007F`를 읽어 비교했고, **이 훅은 한 번도 매치한 적이 없다**(전체 실행에서 호출 15번 x 8행,
  매치 0). 지도 행은 그래서 늘 원본 한자로 남았고, 그것을 `KOREAN16`이 한글로 보이게 하고 있었다.
  v0.6.13의 `本塁` 예약도 그 상태를 전제로 한 것이다.
- **고친 방법.** `ldx z_buf`로 행 주소를 X에 넣고 읽는다. 한 줄이다. 이제 여덟 행이 전부 라벨 풀로
  그려져 `다음 대전 상대`·`이번 대회까지의 성적 [10시합]`·`타율`·`방어율`·`본루타`·`도루`·`실책`이
  모두 제대로 나온다.
- **바뀐 것 하나.** 이 화면은 배경에 `物語`·`熱血` 무늬가 깔려 있는데 그 무늬도 16x16 글리프다. 라벨을
  여덟 개 더 그리게 되면서 풀이 그중 일부를 가져가 무늬가 달라졌다. 그 무늬까지 예약하려면 35타일이 더
  필요하고(예약 237 -> 272, 풀 275 -> 240) 그러면 모드 선택 화면이 깨진다. 실제 크기에서 무늬 차이는
  거의 보이지 않아 본문을 살리는 쪽을 골랐다.
- **`本塁` 예약은 남겨 둔다.** 장면을 다시 들어가면 스크립트가 cmd 16으로 다시 압축을 풀어 원본 워드가
  돌아오므로 `map_labels`가 매번 잡는다. 다만 어떤 경로로든 못 잡으면 글리프 교체로 되돌아가므로,
  8타일짜리 안전판으로 둔다.

### 라벨 풀과 16x16 한자 글리프는 같은 타일을 쓴다 (v0.6.13)

- **증상.** 다음 상대 화면에서 `본루타`가 `ㄷㅜ타`로, `도루`가 `도ㅜ`로, `[10시합]`의 닫는 괄호가 `[`로
  나왔다. v0.6.9 빌드에서도 (다른 글자로) 깨졌으니 오래된 것이다.
- **원인.** 이 화면의 글자는 라벨 풀이 그린 것이 **아니다.** 화면의 타일맵은 스크립트 명령 **10**으로
  올라가는데 우리 훅은 명령 1C만 본다. 그래서 `map_labels`가 이 행들을 한 번도 못 잡고
  (`hits=0`, 호출 15번 x 8행 전부 불일치), 행은 원본 한자 그대로 남아 **`kanji16.KOREAN16`의 글리프
  교체**로 한글이 된다. 그런데 16x16 한자 글리프가 사는 타일 `0x100`~`0x2FF`가 바로 라벨 풀의 자리다
  (`L_POOL0` = `$0100`, `POOL_TILES` = 512). 메뉴 라벨을 그리다 풀이 한 바퀴 돌면 그 글리프를 덮어쓴다.
- **어떻게 잡았나.** 라벨이 그려질 때마다 (id, `L_NEXT`)를 WRAM 링에 적어 두고 화면이 뜬 뒤 덤프했다.
  29건 전부 메뉴·VS 화면 라벨이었고 이 화면의 행은 **하나도 없었다.** 그리고 타일맵은 멀쩡한데 타일 그림만
  틀렸다는 것과 맞물려, 글자가 풀이 아니라 글리프에서 온다는 결론이 나왔다.
- **고친 방법.** `本`과 `塁`를 `RESERVED_GLYPHS`에 넣어 풀이 그 8타일을 건너뛰게 했다. 닫는 괄호도 같이
  살아났다.
- **왜 8타일만인가.** 지도 행(bank `7F`) 전부의 한자를 예약해 보니 예약이 229 → 289타일로 늘고 풀이
  283 → 223타일로 줄어, 이번에는 **모드 선택 화면이 깨졌다.** 그 화면 하나가 라벨 22개에 `L_NEXT`를
  435까지 쓴다. 512칸을 두 용도가 나눠 쓰는 구조라, 예약은 실제로 깨지는 글리프만 최소로 한다.
- **남은 것.** 같은 화면의 `次の対戦相手`는 여전히 `유 대전상대`로 나온다. 글리프 교체는 글리프 하나에
  음절 하나(또는 미리 정한 두 음절)만 줄 수 있는데, `次`는 응원 메뉴의 `[野次]` -> `[야유]` 때문에 이미
  `유`로 잡혀 있다. 제대로 고치려면 명령 10 업로드를 훅해서 `map_labels`가 이 행들을 잡게 해야 한다
  (`labels.csv`의 `map_93B7BC_01C8`에 번역은 이미 있다).
...
- 한 메시지는 최대 2줄, 한 줄은 **248px**(한글 약 20자)이다. 빌드가 넘치는 문자열을 알려 준다.
- **창 너비는 메시지마다 다르고, 스크립트에는 안 적혀 있다.** 게임이 인쇄 직전에 잡는 `T_X`(시작 열)로
  정해진다. 그래서 실측했다: `kbb_start`에서 (문자열 포인터, `T_X`)를 WRAM에 찍고 포인터를 스크립트
  뱅크와 대조해 id로 되돌렸다. 확인된 창은 셋이다.

  | `T_X` | 폭 | 쓰이는 곳 | 실측된 id |
  |------|-----|-----------|-----------|
  | 1 | 248px | 대사 창 | 도달한 모든 스토리 대사 (`story_001`~`054` 등) |
  | 6 | 208px | 안내역(초상화)의 메뉴 창 | `story_121`, `168`, `169` |
  | 7 | 200px | 경기 전 메뉴 창 | `story_437`, `451` |

  좁은 창에서 **본 적이 있는** id만 그 폭으로 검사하고(`build.NARROW_WINDOWS`), 나머지는 248px로 둔다.
  v0.6.15 전에는 모든 `story_*`를 208px로 재서, 248px 창에 멀쩡히 들어가는 대사 다섯 개를 매번 경고로
  찍고 있었다. 새로 좁은 창에서 뜨는 메시지를 발견하면 같은 방법으로 재서 표에 넣는다.
...
한 가지 적어 둘 것: 다음 상대 화면의 두 번째 방문 이후는 **헤드리스로 확인하지 못했다.** 자동 플레이가
경기를 끝내지 못해 그 화면에 다시 도달할 수 없다(메모의 "auto-play cannot finish a match"). 장면이
다시 로드되면 스크립트가 압축을 다시 푸는 구조라 첫 방문과 같아야 하지만, 실제 플레이로 한 번
확인하면 좋다.
