...
**현재 버전: v0.4.1 (인게임 텍스트 분석, 라벨 엔진 안정화)**
...
| v0.5 | 경기 중 텍스트(중계 창, HUD 이름, 투구·타격 메뉴)의 한글화. 인게임 VRAM이 가득 차 있어 8x8 한글 폰트 방식 필요 | 분석 완료, 구현 예정 |
| v0.6 | 전체 플레이 검증과 번역 다듬기, 그림 문자·타이틀 로고 | 예정 |
...
라벨 85개가 1차 번역되어 있습니다. 성 테이블의 빈 항목 148개는 원본에서도 쓰이지 않는 자리입니다.
...
### 인게임 텍스트 조사 결과 (v0.5 준비)

- 경기 중 중계 창은 별도 렌더러(`$04:CAF4`, 뱅크 04)가 그린다. 스크립트 뱅크 디렉터리(`$06:800E`)에서 스토리 테이블을
  찾아 한 프레임에 한 글자씩 VRAM 쓰기 큐(`$80F0FD`: X=WRAM 소스, Y=VRAM 워드 주소, A=바이트 수, 호출 시점의
  데이터 뱅크가 DMA 소스 뱅크가 됨)로 타일맵에 직접 쓴다. 변수 치환(선수·팀·아이템 이름, 숫자)도 같은 루틴 안에 있다.
- 이 렌더러를 대사 창과 같은 방식으로 후킹하는 코드(`kbb_game_start/main/nested`)는 작성되어 있으나
  `build.py`의 `GAME_TEXT = False`로 꺼져 있다. 경기 화면에서는 VRAM 64 KB가 배경 타일, 64x64 타일맵(`$E000`~`$FFFF`),
  스프라이트로 모두 사용 중이어서 16px 글리프 캐시를 둘 자리가 없기 때문이다. 켜면 BG1 타일맵을 덮어써 화면이 깨진다.
- 경기 중 HUD의 선수 이름은 `$0E:F7B6`이 성 테이블(`$06:8018`)을 읽어 8x8 가나 타일 행으로 만들고, 투구·타격 메뉴와
  성적 오버레이도 8x8 폰트를 직접 쓴다. 따라서 인게임 텍스트는 8x8 폰트 영역(`$C000`~`$CFFF`, 256타일)을 재구성하는
  방식으로 가야 한다. 계획: 경기 중에만 쓰이는 8x8 한글 폰트(Galmuri7 기반, 인게임 문자열에서 실제 쓰이는 음절만 수록)로
  폰트 블록을 교체하고, 성 테이블 사본과 메뉴 문자열을 그 인코딩으로 다시 만든 뒤, 중계 창은 남는 폰트 타일로 16px 캐시를 둔다.
...
- 렌더러 작업 영역을 게임이 쓰지 않는 WRAM으로 옮겼다(`$7E:1400` 직접 페이지, `$7E:8800`/`$9000`/`$A400` 버퍼).
  이전 위치(`$7E:B200`~`$D2FF`)는 경기 중 화면 지우기의 0 채움 소스(`$7E:CF73`)와 겹쳐 경기 화면을 깨뜨렸다.
- 라벨 타일은 이제 풀 이미지 없이 타일 단위 업로드 링(32개, NMI당 24개 전송)으로 VRAM에 올리고,
