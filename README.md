...
| v0.7 | 신문 기사 표제 등 남은 그림 문자, 전체 플레이 검증과 번역 다듬기 | 예정 |
...
- 로고 글자는 `assets/title_logo.png`(제공받은 원화)를 따라 그린다(v0.6.3). 원화의 색을 흰 종이·검은 외곽선·파랑·
  초록·빨강 다섯 갈래로 나누고, 갈래마다 따로 축소한 뒤 외곽선 -> 색 순서로 얹는다. 한 번에 축소하면 1픽셀
  외곽선이 채움색에 묻히기 때문이다. 흰색은 종이이므로 언제나 투명 배경으로 둔다.
- 로고 위에 걸친 모자와 공 그림은 원화에서 지우고(`ART_ERASE`) 글자만 옮긴다. 둘째 줄의 낱말 경계는 원화에서
  색이 나타나는 열을 재서 정한다(`ART_WORDS`). 경계를 잘못 잡으면 "볼"의 오른쪽이 빨간 팔레트로 넘어간다.
...
- 블롭 표는 검사 워드를 원본 시트에서 고른다. 타이틀 시트와 게임 오버 시트는 업로드 범위가 겹치므로
  검사 워드가 맞는 쪽에만 복사된다.
- 결과 화면의 `勝ち!` / `負け!!`는 16x16 글리프라 勝→승, ち→리, 負→패, け→배로 매핑했다.
