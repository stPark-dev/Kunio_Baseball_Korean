...
| v0.7 | 남은 그림 문자(엔딩 `おわり`, 신문 기사 표제), 대사 전환 한 프레임 깨짐, 전체 플레이 검증과 번역 다듬기 | 예정 |
...
v0.7로 넘기는 것들입니다. 둘 다 재현 방법이 확인되어 있습니다.
...
스토리를 지면 나오는 엔딩 카드가 일본어 그대로다. 석양본과 야경본 두 가지가 있다. 랜덤 입력으로
돌린 시드 다섯 중 넷이 이 화면에서 끝났으므로 플레이어가 반드시 보게 된다.
