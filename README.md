...
| 연습 시합 팀 선택 격자의 팀 이름 (열혈·하나조노·호료 …) | 라벨 풀의 8px 폰트 모드로 32px 칸에 출력 (`labels.csv`의 `grid_` 행) |
...
- 팀 선택 격자 이름은 라벨 풀에 8px 폰트 모드를 추가해 그린다: 문자열 앞의 `$01` 바이트가 표식이고(`grid_` 행),
  `label_width`/`label_paint`가 음절당 8px로 재고 `blit8`로 8x8 글리프를 칸 세로 가운데(4~11행)에 찍는다.
  전용 글리프 슬롯(`grid.py`, `teams.csv`)은 제거했다. 슬롯이 8x16 숫자 쌍·괄호 글리프를 덮어 `[10試合]`, `.225`가 깨지던
  문제도 함께 사라졌다.
