...
| v0.7 | 대사 전환 한 프레임 깨짐, 전체 플레이 검증과 번역 다듬기 | 예정 |
...
### 대사 전환 때 한 프레임 깨짐 (미해결)

- **증상.** 대사가 한 줄 넘어갈 때마다 정확히 한 프레임 동안 위쪽 검은 띠와 장소 제목이 사라지고
  캐릭터 스프라이트도 빠진다. 다음 프레임에 스스로 돌아온다.
- **재현.** `tools/emu/glitch.py`. 스토리 도입부 6400~15000 프레임에서 17번 잡힌다. 같은 구간을 원본
  롬으로 재면 0번이므로 우리 패치가 원인이 맞다. 다만 v0.6.4를 그대로 빌드해도 17번이라 이번 판에서
  생긴 것은 아니다.
- **알아낸 것.** 깨지는 프레임에 게임의 **태스크 스케줄러** 상태($0000~$000B, $004D/$004F/$0050)가
  잠깐 달라졌다가 되돌아온다. 즉 화면 일부를 그리는 태스크가 한 프레임 돌지 않는다. 순서는
  `kbb_start`(메시지 시작) → 세 프레임 뒤 깨짐 → 그 프레임의 NMI가 새 메시지의 첫 열 업로드.
- **배제한 것.** NMI 훅의 열 업로드 DMA(4열로 줄여도, 아예 빼도 그대로), 타일 업로드 링(`RING_DRAIN`을
  24에서 4까지 낮춰도 그대로), VMAIN($2115) 복구, `kbb_start`의 타일맵 채우기 양(1/4로 줄여도 그대로).
- **주의.** NMI 훅에서 작업을 덜어내는 실험은 대부분 렌더러를 멈춰 게임이 검은 화면이 된다. 그러면
  대사가 아예 안 나와서 "깨짐 0"이 나오므로, 변형을 잴 때는 **반드시 같은 프레임의 화면을 같이 찍어
  게임이 살아 있는지 확인해야 한다.**
- **다음.** Mesen 2의 이벤트 뷰어와 트레이스 로거로 그 한 프레임에 어떤 태스크가 빠지는지, vblank
  안에서 무엇이 밀리는지 직접 본다. 헤드리스 하네스로는 거기까지 보이지 않는다.
