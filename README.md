...
| VS 화면 제목(열혈야구대회·제 n 회전·대), 경기 전 세로 메뉴(선발멤버 변경·타순변경·수비변경·선수정보) | 16x16 한자 글리프를 한글 음절로 교체 (`kanji16.KOREAN16`) |
| 연습 시합 팀 선택 격자의 팀 이름 (열혈·하나조노·호료 …) | 전용 16x16 슬롯에 8px 한글로 그려 출력 (`translations/teams.csv`) |
...
| 스토리 모드 다음 상대 화면(다음 대전 상대, 이번 대회까지의 성적, 타율·방어율·본루타·도루·실책), 학교 전체 이름 | 16x16 글리프 교체(한 글리프에 8px 두 음절) |
| 스토리 장면 상단의 장소 제목(열혈고교, 니지가오카2초메) | 장면 그래픽 압축 해제 버퍼를 DMA 직전에 훑어 글리프 타일을 한글 타일로 바꿈 (`translations/sprtext.csv`) |
| 스토리 1장 타이틀 카드(열혈고교 스토리 / 『스가타군의 연애근성 이야기』 편) | 비압축 타일셋의 글자 타일을 다시 그림 (`translations/titles.csv`) |
| 경기 중 타임 메뉴(투구 메뉴에서 START): 선수 교대·수비 교대·아이템 사용·선수 정보·응원, 선수 목록, 취소/결정 | 16x16 글리프 교체 + 경기 중 8x8 캐시 |
| 2장 이후의 장소 제목·타이틀 카드, 토너먼트 화면, 이름 입력 | **일본어 원본 유지** (해당 장면에 도달하면 같은 표에 추가) |
| 타이틀 로고, 그림 문자 | 미착수 |
...
라벨 85개, 투구·타격 메뉴 항목 141개(`translations/ingame.csv`), 팀 선택 격자 이름 13개(`translations/teams.csv`)가 1차 번역되어 있습니다. 성 테이블의 빈 항목 148개는 원본에서도 쓰이지 않는 자리입니다.
...
│       ├── grid.py           # 연습 시합 팀 선택 격자: 이름을 8px 한글로 그린 16x16 슬롯으로 교체
...
├── translations/teams.csv    # 팀 선택 격자 이름: id, offset, japanese, korean
...
### 16x16 글리프 교체 (v0.5.1, `kanji16.KOREAN16`, `tools/kbb/grid.py`)
...
- 다음 상대 화면·개인 성적 화면의 16x16 텍스트는 워드 행이나 글리프 문자열로 저장돼 있지 않아(압축 타일맵으로 추정)
  글리프 교체로 처리했다. 훈독 학교 이름(花園=하나조노)은 한 글리프에 Galmuri7 두 음절을 세로 2배로 넣는 방식으로
  花→하나, 園→조노처럼 대응시켰다. 園은 花園·夢園을 따르므로 学園은 "학원조노", 谷花는 "타니하나"로 읽히는 타협이 있다.
...
- 표는 장면 실행 중 덤프한 버퍼에서 만든다. 새 장소가 나오면 `sprtext.extract(buf, (tl, tr, bl, br))`로 사분면 hex를 뽑아
  `translations/sprtext.csv`에 한 줄 추가하면 된다. 버퍼 끝이 정확히 `$10000`인 시트가 있어 끝 주소는 16비트에서 넘치지
  않게 마지막 타일 주소로 잡는다.
