...
### 다음 상대 화면의 라벨이 다른 글자로 나온다 (미해결)

- **증상.** 스토리 모드의 다음 상대 화면에서 `다음 대전 상대`가 `유 대전 상대`로, `본루타`가 `ㄷㅜ타`로,
  `도루`가 `도ㅜ`로, `[10시합]`의 닫는 괄호가 `[`로 나온다. `타율`·`방어율`·`실책`은 멀쩡하다.
  v0.6.9 빌드에서도 (다른 글자로) 깨지므로 예전부터 있던 것이다.
- **재현.** 스토리 모드를 시작해(START x4, 이후 A를 120프레임마다) 약 18500프레임. 이 화면은 두 번째
  경기 앞에 나온다.
- **알아낸 것.** 타일맵은 멀쩡하다. 같은 음절은 같은 타일을 가리키고 칸 배치도 원본과 같다. **타일 안의
  그림이 틀렸다.** 16x16 라벨 풀의 캐시(`LMAP`, 36칸 / `POOL_TILES` 512타일)는 다 차면
  `label_render`가 `L_MAPN`과 `L_NEXT`를 **둘 다 0으로 되감는다.** 이미 화면에 그려진 라벨의 타일맵은
  그대로 그 타일을 가리키므로, 되감긴 뒤에 그려지는 라벨이 같은 타일을 받아 덮어쓴다.
- **확인한 증거.** 그 화면에서 `$7E:14CE`(`L_MAPN`)가 24바이트 = **네 칸**뿐이고, 그 네 칸의 id는
  20·35·34·16 (`호료고교`·`후공`·`선공`·`호료고교`)이다. 화면의 지도 라벨(id 107~128)은 **하나도 캐시에
  없다.** 즉 지도 라벨이 먼저 그려진 뒤 캐시가 되감겼고, 뒤이어 그려진 `선공`·`후공`·`호료고교`가 그
  타일을 가져갔다.
- **해 본 것.** `pool_setup`이 장면 번호(`$7E:0043`)가 바뀌면 캐시를 비우게 해 봤다. `본루타`·`도루`는
  고쳐졌지만 이번에는 `타율`·`방어율`이 깨졌다. 장면 번호가 바뀌는 시점과 라벨이 그려지는 시점이 한
  프레임 안에서 엇갈려서, 되감기는 자리만 옮겨졌다. 개선이라고 증명할 수 없어 되돌렸다.
- **다음.** 풀을 "쓰고 버리는 할당기"가 아니라 **화면에 아직 쓰이는 타일을 아는 할당기**로 바꿔야 한다.
  후보: (a) 지도 라벨처럼 장면당 한 번만 그려지는 라벨에 전용 타일 구간을 준다, (b) 캐시 칸 수를 늘려
  타일 풀(512)이 먼저 차게 만들고 되감기를 타일 풀 소진으로 일원화한다($7E:B768 이후 WRAM이 비어 있는지
  먼저 재야 한다), (c) 한 프레임 단위로 라벨 렌더 순서를 볼 수 있는 추적이 필요하면 Mesen 2를 쓴다.
