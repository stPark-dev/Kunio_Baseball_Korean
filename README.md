...
| v0.7 | 남은 그림 문자(엔딩 `おわり`, 우승 `おめでとう`, 신문 기사 표제), 대사 전환 한 프레임 깨짐, 전체 플레이 검증과 번역 다듬기 | 예정 |
...
v0.7로 넘기는 것들입니다. 셋 다 재현 방법이 확인되어 있습니다.
...
게임 오버 시트(`$9B:8000`, 화면 넷이 공유)에 같이 들어 있는데 v0.6.2가 `げーむ おーばー`와
`おつかれさま`만 바꿨다. 같은 시트라 `titlelogo.gameover` / `titlelogo.thanks` 옆에 같은 방식으로
붙이면 된다.
