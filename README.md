...
**현재 버전: v0.6.13 (다음 상대 화면의 본루타·도루 깨짐 수정)**
...
### 다음 상대 화면의 `다음 대전 상대` (미해결)

- `次の対戦相手` 행이 `유 대전상대`로 나온다. 나머지(`본루타`·`도루`·괄호)는 v0.6.13에서 고쳤다.
- 이 행은 `kanji16.KOREAN16`의 글리프 교체로 그려지는데, `次`가 응원 메뉴의 `[野次]` -> `[야유]`에
  이미 `유`로 배정돼 있어 같은 글리프에 `다음`을 줄 수 없다.
- 제대로 고치려면 이 화면의 타일맵을 올리는 **스크립트 명령 10**을 훅해서 `map_labels`가 행을 잡게
  해야 한다. 번역은 `labels.csv`의 `map_93B7BC_01C8`에 이미 들어 있다. 신문 표제(v0.6.8)와 같은
  부류의 문제다 — 그쪽은 명령 06이었다.

#### 명령 10 경로에 대해 알아낸 것

- 장면 VM의 명령 분기표는 `$80:8232`(`JMP ($8232,X)`, X = 명령 x 2)이고 **명령 10은 `$80:85FE`**이다.
  인자는 8바이트: `$0001,Y`->슬롯+8(VRAM 워드), `$0003,Y`->슬롯+0, `$0005,Y`와 `$0006,Y`를 겹쳐 써서
  슬롯+`$0E`..`$10`에 24비트 소스 포인터를 만든다. 우리 화면은 `$7F:9800`이다.
- 핸들러는 업로드를 직접 하지 않고 **태스크 `$91:AC2C`**를 띄운다. 태스크는 소스를 0x40바이트씩
  `$80:F0FD`(VRAM 큐)로 보낸다. 그래서 cmd 1C 훅(`kbb_sheet2`)도, `$81:858A` DMA 훅도 닿지 않는다.
- **태스크는 RTL로 들어오므로 시작이 `$91:AC2C`다.** 바로 앞 `$91:AC2B`의 `PLB`는 위 루틴의
  `JMP $AB6F` 오퍼랜드 끝바이트다. 거기부터 5바이트를 덮으면 그 점프가 깨져 게임이 부팅 직후 검은
  화면이 된다(한 번 겪었다). 훅은 `4B AB C2 30` 네 바이트만 바꾸면 되고, 우리 코드에서 `PHK`는 우리
  뱅크를 밀어 넣으므로 `pea $9191 / plb / plb`로 태스크 뱅크를 직접 넣어야 한다.
- 그 자리에 훅을 달고 `S_LASTSRC = $0E`로 `map_labels`를 부르면 **게임은 정상 부팅하지만 행은 여전히
  안 바뀐다.** 계측해 보면 훅은 13번 불리고, `S_LASTSRC`는 `9800`, `$7F:99C8`의 워드는 `2E80`으로
  `MAPLABELS` 첫 항목이 기다리는 값과 **일치**하며, BG3 이름 베이스도 0이라 `pool_ready`도 통과한다.
  그런데도 타일맵은 원본 한자 타일 그대로다. 즉 막히는 곳은 검사 워드가 아니라 `map_labels` 안쪽
  (`label_prepare` / `label_write_buf`)이다. 다음 사람은 거기부터 계측하면 된다.
