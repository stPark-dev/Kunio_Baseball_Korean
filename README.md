...
| 대회 화면의 학교 이름 글리프 | 같은 서명 교체 표에 추가 (스크린샷 미확인) |
| 신문 기사 장면의 표제 그림 문자, 이름 입력 | **일본어 원본 유지** |
| 타이틀 로고, 그림 문자 | 미착수 |
...
- 대회 화면의 학교 이름은 별도 16x16 글리프 시트(`$AB:C3EC`, 잉크 1·그림자 5·배경 0)를 쓴다. 같은 훅이 적용되도록 그 시트의
  한자 31자와 全·チー·米를 표에 넣었다(峰·霧·龍 … 学→학원, 山·院→공백). 스토리 타이틀 카드는 게임 전체에 하나뿐이다. 버퍼 끝이 정확히 `$10000`인 시트가 있어 끝 주소는 16비트에서 넘치지
  않게 마지막 타일 주소로 잡는다.
